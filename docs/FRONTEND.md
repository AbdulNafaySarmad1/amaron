...
## SEO, analytics, bot protection and ads

- **SEO:** every indexable page sets a canonical URL and `hreflang` alternates for all locales (`lib/seo.ts`, `NEXT_PUBLIC_SITE_URL`). Category pages with a query, sort or filter, and all search results, are `noindex, follow`. Product, breadcrumb and organization JSON-LD is built from the same API response the page renders, never invented (no rating without reviews). `/sitemap.xml` is an index of a static-pages file plus product files of 10,000 URLs each, served by route handlers from the API sitemap endpoint; `robots.ts` disallows the API, checkout, orders and Saved.
- **Analytics:** components call `track()` from `lib/telemetry.ts` with GA4-shaped ecommerce events. Providers are pluggable; the only one today pushes to the GTM `dataLayer` and is registered only when `NEXT_PUBLIC_GTM_ID` is a valid container ID. Provider errors are swallowed, so analytics can never break a purchase. Purchase fires once per order.
- **Turnstile:** with `NEXT_PUBLIC_TURNSTILE_SITE_KEY` set, checkout renders an interaction-only widget and sends its single-use token as `X-Turnstile-Token`; the widget resets after every attempt. Unset, nothing loads.
- **Ads:** `<AdSlot placement>` accepts only editorial and category-content placements, so an ad in checkout, the bag or a product decision is a type error. It renders nothing unless the placement is listed in `NEXT_PUBLIC_AD_PLACEMENTS` (comma-separated), and then only reserves labelled space for a provider.
- **CSP:** third-party origins (GTM/GA, Cloudflare challenges) join the policy only when their integration is configured.
...
The BFF refreshes access tokens server-side within a 30-second expiry window and forwards only `Accept`, JSON content type, `Authorization`, and, when supplied, `Idempotency-Key` and `X-Turnstile-Token`. Its route/method allowlist covers cart item reads/writes, checkout confirmation, and order reads; it is not a general API proxy. Mutations and logout require exact canonical Origin and CSRF-token checks. `APP_URL` is required in production and defines that canonical origin; OIDC issuer and optional internal metadata URL remain server-only.
