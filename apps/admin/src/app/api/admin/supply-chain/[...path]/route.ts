...
import { isAllowedSupplyChainOperation, isStateChangingMethod, validCsrfToken, validRequestOrigin } from "@/lib/auth/security";
...
async function proxy(request: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  ...
  if (!isAllowedSupplyChainOperation(request.method, path)) return NextResponse.json({ title: "Not found", status: 404 }, { status: 404 });
  ...
  try {
    const upstream = await fetch(`${requiredEnv("API_URL")}/api/admin/supply-chain${path}${request.nextUrl.search}`, { method: request.method, headers, body: isStateChangingMethod(request.method) ? await request.arrayBuffer() : undefined, redirect: "manual", cache: "no-store", signal: AbortSignal.timeout(15_000) });
    const responseHeaders = new Headers({ "Cache-Control": "no-store" });
    const upstreamType = upstream.headers.get("content-type");
    if (upstreamType) responseHeaders.set("Content-Type", upstreamType);
    return new NextResponse(upstream.body, { status: upstream.status, headers: responseHeaders });
  } catch {
    return NextResponse.json({ title: "Service unavailable", detail: "The supply-chain service could not be reached.", status: 503 }, { status: 503, headers: { "Cache-Control": "no-store" } });
  }
}
