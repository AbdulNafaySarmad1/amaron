...
export async function GET(request: NextRequest) {
  ...
  const upstream = await fetch(`${requiredEnv("API_URL")}/api/search/suggestions?q=${encodeURIComponent(query)}${category ? `&category=${category}` : ""}${isLocale(locale) ? `&locale=${locale}` : ""}`, {
