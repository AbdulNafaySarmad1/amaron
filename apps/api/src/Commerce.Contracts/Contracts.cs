...
public sealed record ProductCardDto(Guid Id, Guid DefaultVariantId, string Slug, string Title, string Brand, ImageDto? PrimaryImage, MoneyDto Price, MoneyDto? ListPrice, decimal Rating, int ReviewCount, string AvailabilityHint, string[] Badges, string Kind, IReadOnlyList<SpecDto> Highlights, string Locale);
...
public sealed record ProductDetailDto(Guid Id, string Slug, string Title, string Brand, string Description, string Category, IReadOnlyList<VariantDto> Variants, IReadOnlyList<AssetDto> Assets, decimal Rating, int ReviewCount, string CategorySlug, string Kind, IReadOnlyList<SpecDto> Specifications, string Locale, string? SeoTitle, string? SeoDescription, string CategoryLocale);
...
public sealed record RelatedProductDto(ProductCardDto Product, string Reason);
public sealed record RelationshipGroupDto(string Type, IReadOnlyList<RelatedProductDto> Items);
public sealed record StorefrontProductDto(ProductDetailDto Product, IReadOnlyList<ProductCardDto> Recommendations, bool IsDegraded, IReadOnlyList<RelationshipGroupDto> Relationships);
